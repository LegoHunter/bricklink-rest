...
class BricklinkRestClientEndpointTest extends BricklistRestClientTest {
    ...
    @Test
    void updateOrder_sendsJsonPayload() {
        stubFor(put(urlEqualTo("/orders/31229149"))
                .withHeader("Content-Type", containing("application/json"))
                .withRequestBody(containing("\"tracking_no\":\"9400150206217640076439\""))
                .withRequestBody(containing("\"date_shipped\":\"2026-04-13T00:00:00.000Z\""))
                .willReturn(okJson("""
                        {
                          "meta": {"code": 200, "message": "OK", "description": "OK"},
                          "data": {"order_id": 31229149}
                        }
                        """)));
        Shipping shipping = new Shipping();
        shipping.setTracking_no("9400150206217640076439");
        shipping.setDate_shipped(ZonedDateTime.parse("2026-04-13T00:00:00Z"));
        Order order = new Order();
        order.setShipping(shipping);
        order.setIs_filed(false);

        Order updatedOrder = bricklinkRestClient.updateOrder("31229149", order).getData();

        assertThat(updatedOrder.getOrder_id()).isEqualTo("31229149");
    }

    @Test
    void updateOrderStatus_sendsJsonPayload() {
        stubFor(put(urlEqualTo("/orders/31229149/status"))
                .withHeader("Content-Type", containing("application/json"))
                .withRequestBody(containing("\"field\":\"status\""))
                .withRequestBody(containing("\"value\":\"SHIPPED\""))
                .willReturn(okJson("""
                        {
                          "meta": {"code": 200, "message": "OK", "description": "OK"},
                          "data": {}
        ...
        bricklinkRestClient.updateOrderStatus("31229149", com.bricklink.api.rest.model.v1.OrderStatus.SHIPPED);
    }
